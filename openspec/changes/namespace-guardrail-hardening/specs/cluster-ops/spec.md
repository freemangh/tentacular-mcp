...
The system SHALL run a series of validation checks for a given namespace and return structured results. Checks SHALL include: API server reachability, namespace existence, RBAC permissions for the workflow ServiceAccount (deployments, services, configmaps, secrets, cronjobs, jobs), and gVisor RuntimeClass availability. Each check result SHALL include name, pass/fail status, and optional warning and remediation text. The system SHALL reject the operation if the target namespace is in the protected set (`tentacular-system`, `kube-system`, `kube-public`, `kube-node-lease`, `default`).
