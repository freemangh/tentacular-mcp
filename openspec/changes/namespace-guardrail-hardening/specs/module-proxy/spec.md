...
- **WHEN** the `module_apply` tool is called with a release name that already has resources in the namespace
...
- **WHEN** the `module_apply` tool is called for a namespace without the managed-by label
...
#### Scenario: Reject for system namespace
- **WHEN** the `module_apply` tool is called with a namespace in the protected set
...
The system SHALL delete all resources labeled with `tentacular.io/release: <release-name>` in the given namespace. The system SHALL verify the namespace is managed by tentacular and SHALL reject the operation if the target namespace is in the protected set or is not managed by tentacular.

#### Scenario: Remove existing release
- **WHEN** the `module_remove` tool is called with `namespace: "dev-alice"` and `release: "my-app"`
...
- **WHEN** the `module_remove` tool is called with a release name that has no matching resources
- **THEN** the system returns a success response with zero resources deleted

#### Scenario: Reject for unmanaged namespace
- **WHEN** the `module_remove` tool is called with a namespace that lacks the managed-by label
...
The system SHALL list all resources in a namespace labeled with `tentacular.io/release: <release-name>` and return their kind, name, and readiness status. The system SHALL verify the namespace is managed by tentacular and SHALL reject the operation if the target namespace is in the protected set or is not managed by tentacular.

#### Scenario: Get status of healthy release
- **WHEN** the `module_status` tool is called with `namespace: "dev-alice"` and `release: "my-app"` and all resources are ready
...
- **WHEN** the `module_status` tool is called and some Deployments have unavailable replicas
- **THEN** the system returns resources with `ready: false` for the unhealthy ones, including a reason string

#### Scenario: Reject for unmanaged namespace
- **WHEN** the `module_status` tool is called with a namespace that lacks the managed-by label
- **THEN** the system returns an error indicating the namespace is not managed by tentacular
